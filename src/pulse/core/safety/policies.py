...
def reply_policy(intent: Intent, ctx: PermissionContext) -> Decision:
    ...
    **判决优先级**:
    ...
    2. ``evidence_keys`` 非空且 profile 给出完整证据 → allow.
    3. ``evidence_keys`` 非空但 profile 缺证据 → ask.
    4. ``evidence_keys`` 为空 → allow；Planner 已把实时事实 / 承诺类消息分流为 escalate.
    """
    args = intent.args
    ...
    if intent.evidence_keys:
        if profile_covers(ctx.profile_view, intent.evidence_keys):
            return Decision.allow(
        ...
        return Decision.allow(
            reason="planner_selected_reply",
            rule_id="job_chat.reply.planner_allow",
        )
...
def send_resume_policy(intent: Intent, ctx: PermissionContext) -> Decision:
    ...
    **判决优先级**: session_approved → allow, 否则 allow。发送 BOSS 简历附件是
    用户启用自动回复后的预定义求职动作, 不代表 Agent 替用户编造新事实。
    """
    args = intent.args
    ...
    return Decision.allow(
        reason="predefined_resume_action",
        rule_id="job_chat.send_resume.predefined_allow",
    )
...
def card_policy(intent: Intent, ctx: PermissionContext) -> Decision:
    ...
    **判决**: 换简历 / 岗位推荐这类预定义求职动作 allow；面试邀约、交换联系方式
    绑定时间或隐私承诺, ask。
    """
    args = intent.args
    card_type = str(args.get("card_type") or "").strip()
    if card_type in {"exchange_resume", "job_recommend"}:
        return Decision.allow(
            reason="predefined_card_action",
            rule_id="job_chat.card.predefined_allow",
        )
